...
public class GetMaterialHistoriesByMaterialQueryHandlerTests
{
    ...
    public async Task Handler_ShouldReturnSuccess_WhenReceivedMaterialHistoriesIsNotNull()
    {
        ...
        var getMaterialHistoriesByMaterialQueryHandler = 
            new GetMaterialHistoriesByMaterialQueryHandler(
                _materialHistoryRepositoryMock.Object,
                _mapperMock.Object);

        _materialHistoryRepositoryMock.Setup(repo =>
                            repo.GetMaterialHistoriesByMaterialNameAndDateAsync(getMaterialHistoriesByMaterialQuery))
                                .ReturnsAsync((new List<Domain.Entities.MaterialHistory>() 
                                { new Domain.Entities.MaterialHistory() }, 1));
        _mapperMock.Setup(mapper => mapper.Map<MaterialHistoryResponse>(It.IsAny<Domain.Entities.MaterialHistory>()))
            .Returns(It.IsAny<MaterialHistoryResponse>);
        var result = await getMaterialHistoriesByMaterialQueryHandler
            .Handle(getMaterialHistoriesByMaterialQuery, default);
